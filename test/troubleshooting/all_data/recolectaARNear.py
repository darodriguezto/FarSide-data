...
# -*- coding: utf-8 -*-
"""
Created on Tue Jul 15 10:37:31 2025
...
def main(year):
    ...
    archivo_salida = os.path.join(carpeta_salida, 'AR_EastLimb_corrX.csv')
    anho=int(year)
    ...
    while current_date <= end_date:
        ...
        for i in range(len(data)):
            ...
            if Nombre_AR not in nombres_vistos:
                if EastLimb.lon.degree <= londecarrington <= EastLimb.lon.degree + 35:
                    ...
                    AR_EastLimb.append([Nombre_AR, date, round(londecarrington, 1), round(latituddelasar, 1)])
        ...
        AR_EastLimb_df = pd.DataFrame(AR_EastLimb, columns=['Designation', 'Date', 'Carrington Longitude', 'Latitude'])
