...
# -*- coding: utf-8 -*-
"""
Created on Wed Oct 30 22:19:17 2024
...
file = f"~/Documentos/GoSA/Far_Side/FarSide-data/test/troubleshooting/all_data/{args.year}/archivo_normalizado_last.csv"
...
plt.savefig(os.path.expanduser(f'~/Documentos/GoSA/Far_Side/FarSide-data/test/troubleshooting/all_data/{args.year}/ambas(semanas_completas)_last.png'))
plt.show()

# Graficar la autocorrelación para ambas series
plot_acf(Near, lags=13, title='Autocorrelación para el # de Nuevas AR')
plt.savefig(os.path.expanduser(f'~/Documentos/GoSA/Far_Side/FarSide-data/test/troubleshooting/all_data/{args.year}/autonear(semanas_completas)_last.png'))
plt.show()

plot_acf(Far, lags=13, title='Autocorrelación para el Strength promedio')
plt.savefig(os.path.expanduser(f'~/Documentos/GoSA/Far_Side/FarSide-data/test/troubleshooting/all_data/{args.year}/autofar(semanas_completas)_last.png'))
plt.show()
...
output_file_lags = os.path.expanduser(f'~/Documentos/GoSA/Far_Side/FarSide-data/test/troubleshooting/all_data/{args.year}/{args.year}_lags_last.csv')
...
try:
    resultados_df = pd.read_csv('resultados_last.csv')
...
resultados_df.to_csv('resultados_last.csv', index=False)
...
plt.savefig(os.path.expanduser(f'~/Documentos/GoSA/Far_Side/FarSide-data/test/troubleshooting/all_data/{args.year}/crosscorrelation_{args.year}(semanas_completas)_last.png'))
plt.show()
